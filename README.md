...
The first step is to shuffle the data. The reason for shuffling is quite obvious and to promote generalization over memorization. Then I wrote down the code for normalization, but decided to commet it out. Normalization doesn't seem to be helping my CNN (Convolutional Neural Network) too much when learning. The last step but not the least is to convert images to grayscale. Converting the image to grayscale reduces the input image size from 32x32x3 to 32x32x1. The colors in a traffic sign is not too important. Hence feeding grayscale data would not hurt the accuracy, but improves processing speed and reduces computer hardware requirement.
